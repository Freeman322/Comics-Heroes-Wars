...
function customSchema:init()
    ...
    -- Flag Example
    statCollection:setFlags({ version = 3.5 })
...
--[[function GetPlayerRaiting(id)
     local hero = PlayerResource:GetSelectedHeroEntity(playerID)
     local id = hero:GetTeamNumber()


end

function IsPlayerLose(teamID)
    local allAncients = Entities:FindAllByClassname("npc_dota_fort")
     for k, fort in pairs(allAncients) do
        if fort:IsAlive() == false
            if fort:GetTeamNumber() == teamID then
                return true
            else
                return false
            end
