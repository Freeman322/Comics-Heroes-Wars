...
function UpdateItem()
{
	...
	if ( m_Item == -1 || Abilities.IsCooldownReady( m_Item ) )
	{
		...
		panel.GetChild(0).GetChild(2).SetHasClass("AbilityButtonInside", false);
		panel.GetChild(0).GetChild(2).style.clip = "radial(50% 50%, 0deg, " + 360 + "deg)";
	}
	else
	{
		panel.SetHasClass( "cooldown_ready", false );
		panel.SetHasClass( "in_cooldown", true );
		panel.GetChild(0).GetChild(2).SetHasClass("AbilityButtonInside", true);
		...
		var cooldown = Abilities.GetCooldown(m_Item);
		var remaining = Abilities.GetCooldownTimeRemaining(m_Item);
		var activated = Abilities.IsActivated(m_Item); 
		var progress = (-360 * (remaining / cooldown)).toString();
		/*$( "#CooldownOverlay" ).style.width = cooldownPercent+"%";
		$.GetContextPanel().style.backgroundImage = 'url("file://{images}/spellicons/item_' + item + '.png")';*/
        panel.GetChild(0).GetChild(2).style.clip = "radial(50% 50%, 0deg, " + progress + "deg)";
	}
	$.Schedule( 0.1, UpdateItem );
}
...
function ItemShowTooltip()
{
	...
	var itemName = Abilities.GetAbilityName( m_Item );
	for (var i=0; i<=6; i++){
		if (Entities.GetItemInSlot( m_QueryUnit, i ) == m_Item)
		{
			$.DispatchEvent( "DOTAShowAbilityInventoryItemTooltip", $.GetContextPanel(), m_QueryUnit, i );		
		}
	}
}
