...
function Util:OnInit(args)
    ...
    Convars:RegisterCommand( "try_drop", Dynamic_Wrap(Util, 'Drop'), "Test", FCVAR_CHEAT )

    local treassures = LoadKeyValues('scripts/items/treassures.kv')
...
function Util:OnHeroInGame(hero)
    ...
    if not hero:GetUnitName() == "npc_dota_hero_meepo" then 
    ...
    if hero:GetUnitName() == "npc_dota_hero_leshrac" then
      ...
      local pudge_donat = ParticleManager:CreateParticle( "particles/econ/courier/courier_onibi/courier_onibi_yellow_ambient_eye_lvl16.vpcf", PATTACH_ABSORIGIN_FOLLOW, hero )
      ParticleManager:SetParticleControlEnt( pudge_donat, 0, hero, PATTACH_POINT_FOLLOW, "attach_eye_r" , hero:GetOrigin(), true )
      local pudge_donat2 = ParticleManager:CreateParticle( "particles/econ/courier/courier_onibi/courier_onibi_yellow_ambient_eye_lvl16.vpcf", PATTACH_ABSORIGIN_FOLLOW, hero )
      ParticleManager:SetParticleControlEnt( pudge_donat2, 0, hero, PATTACH_POINT_FOLLOW, "attach_eye_l" , hero:GetOrigin(), true )

      local mask = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/heroes/hero_tzeentch/tzeentch_armor.vmdl"})
      mask:FollowEntity(hero, true)

      local mask1 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/heroes/hero_tzeentch/tzeentch_cape.vmdl"})
      mask1:FollowEntity(hero, true)

      local mask2 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/heroes/hero_tzeentch/tzeentch_knife.vmdl"})
    ...
    if hero:GetUnitName() == "npc_dota_hero_furion" then
      local mask = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/heroes/hero_dimm/weapon.vmdl"})
      mask:FollowEntity(hero, true)
    end
    ...
    if hero:GetUnitName() == "npc_dota_hero_necrolyte" then
      local mask = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/items/necrolyte/apostle_of_decay_head/apostle_of_decay_head.vmdl"})
      mask:FollowEntity(hero, true)
      mask:SetRenderColor(0, 0, 0)

      local mask2 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/items/necrolyte/apostle_of_decay_weapon/apostle_of_decay_weapon.vmdl"})
      mask2:FollowEntity(hero, true)
      mask2:SetRenderColor(0, 0, 0)
      if Util:PlayerEquipedItem(hero:GetPlayerOwnerID(), "palpatine_shoulder") == true then
        local mask1 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/items/necrolyte/immortal_shoulders/immortal_shoulders.vmdl"})
        mask1:FollowEntity(hero, true)

        local mask1amb = ParticleManager:CreateParticle( "particles/econ/items/necrolyte/necrophos_sullen/necro_sullen_ambient.vpcf", PATTACH_ABSORIGIN_FOLLOW, mask1 )
      else
        local mask3 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/items/necrolyte/apostle_of_decay_shoulder/apostle_of_decay_shoulder.vmdl"})
        mask3:FollowEntity(hero, true)
        mask3:SetRenderColor(0, 0, 0)
      end

      local mask4 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/items/necrolyte/apostle_of_decay_mask/apostle_of_decay_mask.vmdl"})
      mask4:FollowEntity(hero, true)
      mask4:SetRenderColor(0, 0, 0)
    end
    ...
    if hero:GetUnitName() == "npc_dota_hero_elder_titan" then
      ...
      elseif Util:PlayerEquipedItem(hero:GetPlayerOwnerID(), "king_thanos") and not hasCosmetic then
        ...
        SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/heroes/hero_thanos/weap/weapon.vmdl"}):FollowEntity(hero, true)
      end
      
    end
    if hero:GetUnitName() == "npc_dota_hero_shadow_demon" then
      LinkLuaModifier("modifier_sargeras_effect", "modifiers/modifier_sargeras_effect.lua", LUA_MODIFIER_MOTION_NONE)
      LinkLuaModifier("modifier_sargeras_arcana", "modifiers/modifier_sargeras_arcana.lua", LUA_MODIFIER_MOTION_NONE)
      hero:AddNewModifier(hero, nil, "modifier_sargeras_effect", nil)

      if Util:PlayerEquipedItem(hero:GetPlayerOwnerID(), "sargeras_elder_titan") then
        hero:AddNewModifier(hero, nil, "modifier_sargeras_arcana", nil)

        hero:SetOriginalModel("models/heroes/sargeras/sargeras_burning_god/sargeras_elder_titan.vmdl")

        local mask = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/items/elder_titan/harness_of_the_soulforged_arms/harness_of_the_soulforged_arms.vmdl"})
        mask:FollowEntity(hero, true)

        local mask1 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/items/elder_titan/warden_of_the_gong_head/warden_of_the_gong_head.vmdl"})
        mask1:FollowEntity(hero, true)

        local mask2 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/items/elder_titan/world_splitter_shoulder/world_splitter_shoulder.vmdl"})
        mask2:FollowEntity(hero, true)
        --particles/econ/items/doom/doom_weapon_hellthorns_sword/doom_hellthorns_sword.vpcf

        local mask3 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/heroes/sargeras/sargeras_burning_god/sargeras_gorshalach.vmdl"})
        mask3:FollowEntity(hero, true)

        local mask1_particle = ParticleManager:CreateParticle( "particles/econ/items/doom/doom_weapon_hellthorns_sword/doom_hellthorns_sword.vpcf", PATTACH_ABSORIGIN_FOLLOW, mask3 )
        ParticleManager:SetParticleControlEnt( mask1_particle, 0, mask3, PATTACH_POINT_FOLLOW, "attach_sword" , mask3:GetOrigin(), true )
        ParticleManager:SetParticleControl( mask1_particle, 3, Vector(1, 1, 1))
        ParticleManager:SetParticleControlEnt( mask1_particle, 4, mask3, PATTACH_POINT_FOLLOW, "attach_sword" , mask3:GetOrigin(), true )
        ParticleManager:SetParticleControl( mask1_particle, 6, Vector(1, 1, 1))
        ParticleManager:SetParticleControl( mask1_particle, 15, Vector(255, 255, 255))
        ParticleManager:SetParticleControl( mask1_particle, 16, Vector(1, 1, 1))


        local mask2_particle = ParticleManager:CreateParticle( "particles/econ/items/elder_titan/elder_titan_fissured_soul/elder_titan_fissured_soul_weapon.vpcf", PATTACH_ABSORIGIN_FOLLOW, mask3 )
        ParticleManager:SetParticleControlEnt( mask2_particle, 0, mask3, PATTACH_POINT_FOLLOW, "attach_grip" , mask3:GetOrigin(), true )
        ParticleManager:SetParticleControlEnt( mask2_particle, 1, mask3, PATTACH_POINT_FOLLOW, "attach_grip" , mask3:GetOrigin(), true )
        ParticleManager:SetParticleControl( mask2_particle, 3, Vector(1, 1, 1))
        ParticleManager:SetParticleControl( mask2_particle, 4, Vector(1, 1, 1))

        local mask3_particle = ParticleManager:CreateParticle( "particles/econ/items/doom/doom_baphomet_set/doom_skulleye.vpcf", PATTACH_ABSORIGIN_FOLLOW, hero )
        ParticleManager:SetParticleControlEnt( mask3_particle, 0, hero, PATTACH_POINT_FOLLOW, "attach_eye_l", hero:GetOrigin(), true )
        ParticleManager:SetParticleControlEnt( mask3_particle, 1, hero, PATTACH_POINT_FOLLOW, "attach_eye_r", hero:GetOrigin(), true )

        local mask4_particle = ParticleManager:CreateParticle( "particles/econ/courier/courier_golden_doomling/courier_golden_doomling_ambient.vpcf", PATTACH_ABSORIGIN_FOLLOW, hero )
        ParticleManager:SetParticleControlEnt( mask4_particle, 0, hero, PATTACH_POINT_FOLLOW, "attach_hitloc" , hero:GetOrigin(), true )
        ParticleManager:SetParticleControlEnt( mask4_particle, 1, hero, PATTACH_POINT_FOLLOW, "attach_hitloc" , hero:GetOrigin(), true )
        ParticleManager:SetParticleControlEnt( mask4_particle, 2, hero, PATTACH_POINT_FOLLOW, "attach_hitloc" , hero:GetOrigin(), true )
        ParticleManager:SetParticleControl( mask4_particle, 3, Vector(1, 1, 1))
        ParticleManager:SetParticleControlEnt( mask4_particle, 4, hero, PATTACH_POINT_FOLLOW, "attach_hitloc" , hero:GetOrigin(), true )
        ParticleManager:SetParticleControl( mask4_particle, 6, Vector(1, 1, 1))
        ParticleManager:SetParticleControl( mask4_particle, 7, Vector(1, 1, 1))
        ParticleManager:SetParticleControl( mask4_particle, 15, Vector(255, 255, 255))
        ParticleManager:SetParticleControl( mask4_particle, 16, Vector(1, 1, 1))


        local mask5_particle = ParticleManager:CreateParticle( "particles/econ/items/monkey_king/arcana/monkey_king_arcana_fire.vpcf", PATTACH_ABSORIGIN_FOLLOW, hero )
        ParticleManager:SetParticleControlEnt( mask5_particle, 0, hero, PATTACH_POINT_FOLLOW, "attach_hitloc" , hero:GetOrigin(), true )
        ParticleManager:SetParticleControlEnt( mask5_particle, 1, hero, PATTACH_POINT_FOLLOW, "attach_hitloc" , hero:GetOrigin(), true )
        ParticleManager:SetParticleControlEnt( mask5_particle, 2, hero, PATTACH_POINT_FOLLOW, "attach_hitloc" , hero:GetOrigin(), true )
        ParticleManager:SetParticleControlEnt( mask5_particle, 3, hero, PATTACH_POINT_FOLLOW, "attach_hitloc" , hero:GetOrigin(), true )
        ParticleManager:SetParticleControlEnt( mask5_particle, 4, hero, PATTACH_POINT_FOLLOW, "attach_hitloc" , hero:GetOrigin(), true )
        ParticleManager:SetParticleControl( mask5_particle, 5, Vector(0, 0, 0))
        ParticleManager:SetParticleControl( mask5_particle, 6, Vector(0, 0, 0))
        ParticleManager:SetParticleControl( mask5_particle, 7, Vector(0, 0, 0))
      end
    end
    if hero:GetUnitName() == "npc_dota_hero_spectre" then
      hero:SetRenderColor(46, 139, 87)
      local mask = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/dcu_spectre/spectre_head_final.vmdl"})
      mask:FollowEntity(hero, true)

      local mask1 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/items/silencer/bts_final_utterance_shoulder/bts_final_utterance_shoulder.vmdl"})
      mask1:FollowEntity(hero, true)

      local mask2 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/items/silencer/whispertribunal_arms/whispertribunal_arms.vmdl"})
      mask2:FollowEntity(hero, true)
      if Util:PlayerEquipedItem(hero:GetPlayerOwnerID(), "spectre_armor") == true then
        local mask3 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/items/silencer/silent_guardian_belt/silent_guardian_belt.vmdl"})
        mask3:FollowEntity(hero, true)
      end
    end
    ...
    if hero:GetUnitName() == "npc_dota_hero_phantom_assassin" then
      if Util:PlayerEquipedItem(hero:GetPlayerOwnerID(), "reality_paradox") == true then
        local mask = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/heroes/hero_katana/reality_paradox/reality_paradox_head.vmdl"})
        mask:FollowEntity(hero, true)

        local mask1 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/heroes/hero_katana/reality_paradox/reality_paradox_shoulder.vmdl"})
        mask1:FollowEntity(hero, true)

        local mask2 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/items/phantom_assassin/creeping_shadow_belt/creeping_shadow_belt.vmdl"})
        mask2:FollowEntity(hero, true)

        local mask3 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/items/phantom_assassin/phantom_knight_belt/phantom_knight_belt.vmdl"})
        mask3:FollowEntity(hero, true)

        local mask4 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/items/phantom_assassin/toll_of_the_fearful_aria_belt/toll_of_the_fearful_aria_belt.vmdl"})
        mask4:FollowEntity(hero, true)

        local mask5 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/items/phantom_assassin/toll_of_the_fearful_aria_belt/toll_of_the_fearful_aria_belt.vmdl"})
        mask5:FollowEntity(hero, true)

        local mask6 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/items/phantom_assassin/creeping_shadow_back/creeping_shadow_back.vmdl"})
        mask6:FollowEntity(hero, true)

        local mask7 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/heroes/hero_katana/reality_paradox/reality_paradox_weapon.vmdl"})
        mask7:FollowEntity(hero, true)

        local pudge_donat = ParticleManager:CreateParticle( "particles/econ/items/sven/sven_weapon_ice/sven_ice_sword.vpcf", PATTACH_ABSORIGIN_FOLLOW, hero )
        ParticleManager:SetParticleControlEnt( pudge_donat, 0, mask7, PATTACH_POINT_FOLLOW, "attach_sword" , mask7:GetOrigin(), true )
        ParticleManager:SetParticleControl( pudge_donat, 1, Vector(500, 500, 0) )
        ParticleManager:SetParticleControl( pudge_donat, 2, Vector(500, 500, 0) )

        local body = ParticleManager:CreateParticle( "particles/econ/items/monkey_king/arcana/monkey_king_arc_death.vpcf", PATTACH_ABSORIGIN_FOLLOW, hero )
        ParticleManager:SetParticleControlEnt( body, 0, hero, PATTACH_POINT_FOLLOW, "attach_hitloc" , hero:GetOrigin(), true )
        ParticleManager:SetParticleControlEnt( body, 1, hero, PATTACH_POINT_FOLLOW, "attach_eye_l" , hero:GetOrigin(), true )
        ParticleManager:SetParticleControlEnt( body, 2, hero, PATTACH_POINT_FOLLOW, "attach_eye_r" , hero:GetOrigin(), true )
        ParticleManager:SetParticleControlEnt( body, 3, hero, PATTACH_POINT_FOLLOW, "attach_hitloc" , hero:GetOrigin(), true )
        ParticleManager:SetParticleControl( body, 5, Vector(0, 0, 0) )
        ParticleManager:SetParticleControl( body, 6, Vector(0, 0, 0) )
        ParticleManager:SetParticleControl( body, 7, Vector(0, 0, 0) )
      else
        LinkLuaModifier("modifier_katana_effect", "modifiers/modifier_katana_effect.lua", LUA_MODIFIER_MOTION_NONE)
        hero:AddNewModifier(hero, nil, "modifier_katana_effect", nil)

        local mask = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/items/phantom_assassin/creeping_shadow_shoulder/creeping_shadow_shoulder.vmdl"})
        mask:FollowEntity(hero, true)

        local mask1 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/items/phantom_assassin/creeping_shadow_belt/creeping_shadow_belt.vmdl"})
        mask1:FollowEntity(hero, true)

        local mask2 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/items/phantom_assassin/creeping_shadow_back/creeping_shadow_back.vmdl"})
        mask2:FollowEntity(hero, true)

        if Util:PlayerEquipedItem(hero:GetPlayerOwnerID(), "outwordly_paradox") == true then
          local mask3 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/items/phantom_assassin/pa_ti8_immortal_head/pa_ti8_immortal_head.vmdl"})
          mask3:FollowEntity(hero, true)
          
          ParticleManager:CreateParticle( "particles/econ/items/phantom_assassin/pa_ti8_immortal_head/pa_ti8_immortal_head_ambient.vpcf", PATTACH_ABSORIGIN_FOLLOW, mask3 )
        else
          local mask3 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/items/phantom_assassin/toll_of_the_fearful_aria_head/toll_of_the_fearful_aria_head.vmdl"})
          mask3:FollowEntity(hero, true)
        end

        local mask4 = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/katana/katana.vmdl"})
        mask4:FollowEntity(hero, true)
      end
    end
    ...
    if hero:GetUnitName() == "npc_dota_hero_obsidian_destroyer" then
      if Util:PlayerEquipedItem(hero:GetPlayerOwnerID(), "tribunal_supreme_harvester") == true then
        hero.spear = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/heroes/hero_livingtribunal/tribunal_immortal_spear.vmdl"})
        hero.spear:FollowEntity(hero, true)

        local spearPFX = ParticleManager:CreateParticle( "particles/tribunal/tribunal_iimortal_spear_ambient.vpcf", PATTACH_ABSORIGIN_FOLLOW, hero.spear )
        ParticleManager:SetParticleControlEnt( spearPFX, 0, hero.spear, PATTACH_POINT_FOLLOW, "attach_end" , hero.spear:GetOrigin(), true )
        ParticleManager:SetParticleControlEnt( spearPFX, 1, hero.spear, PATTACH_POINT_FOLLOW, "attach_end" , hero.spear:GetOrigin(), true )
        ParticleManager:SetParticleControlEnt( spearPFX, 2, hero.spear, PATTACH_POINT_FOLLOW, "attach_end" , hero.spear:GetOrigin(), true )
        ParticleManager:SetParticleControl( spearPFX, 3, Vector(1, 1, 1) )
        ParticleManager:SetParticleControl( spearPFX, 15, Vector(246, 255, 0) )
        ParticleManager:SetParticleControl( spearPFX, 16, Vector(1, 0, 0) )
      else
        local staff = SpawnEntityFromTableSynchronous("prop_dynamic", {model = "models/heroes/hero_livingtribunal/tribunal_staff.vmdl"})
        staff:FollowEntity(hero, true)
      end
    end
    ...
    Convars:RegisterCommand("wingame", function(command, team )
      if Util:PlayerHasAdminRules(Convars:GetCommandClient():GetPlayerID()) then  
        if tonumber(team) == DOTA_TEAM_GOODGUYS then
          _G.Dire_fort:ForceKill(false)
          GameRules:SetGameWinner(DOTA_TEAM_GOODGUYS)
        else
          _G.Radiant_fort:ForceKill(false)
...
function Util:CheckGameState()
  if GameRules.Players[DOTA_TEAM_GOODGUYS] then
    ...
    for _, pID in pairs(GameRules.Players[DOTA_TEAM_GOODGUYS]) do
      if not PlayerResource:GetConnectionState(pID) == DOTA_CONNECTION_STATE_ABANDONED then
        state = false break
      end
    end

    if state then GameRules:MakeTeamLose( DOTA_TEAM_GOODGUYS ) end 
  end 
  ...
  if GameRules.Players[DOTA_TEAM_BADGUYS] then
    ...
    if state then GameRules:MakeTeamLose( DOTA_TEAM_BADGUYS ) end 
  end 
end
