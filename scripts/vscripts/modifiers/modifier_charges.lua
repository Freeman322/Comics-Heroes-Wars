...
amodifier_charges = class({})

if IsServer() then
    function modifier_charges:Update()
        if self:GetDuration() == -1 then
			local octarine = 1
			if self:GetParent():HasModifier("modifier_item_octarine_core") or self:GetParent():HasModifier("modifier_item_bloodstaff") then
				octarine = 0.75
			end
			if self:GetParent():HasModifier("modifier_item_adamachi_core") or self:GetParent():HasModifier("modifier_item_aether_medallion") then
				octarine = 0.70
			end
