...
function zoom_time_shift:OnSpellStart()
    ...
    if PlayerResource:GetSteamAccountID(self:GetCaster():GetPlayerOwnerID()) == 102494627 or PlayerResource:GetSteamAccountID(self:GetCaster():GetPlayerOwnerID()) == 186759206 then duration = duration * 2 radius = radius * 2 MAX_SPEED = 15000 self:EndCooldown() self:StartCooldown(self:GetCooldown(self:GetLevel()) / 2) end
...
function modifier_proffesor_zoom_time_shift:CheckState()
    if (IsServer() and (PlayerResource:GetSteamAccountID(self:GetParent():GetPlayerOwnerID()) == 102494627 or PlayerResource:GetSteamAccountID(self:GetParent():GetPlayerOwnerID()) == 186759206)) then return {[MODIFIER_STATE_MAGIC_IMMUNE] = true} end
    return
end
