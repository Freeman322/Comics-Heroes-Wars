...
function genos_incineration:GetAOERadius()
  return self:GetSpecialValueFor("radius")
end

function genos_incineration:OnSpellStart()
  local point = self:GetCursorPosition()
  local enemies = FindUnitsInRadius( self:GetCaster():GetTeamNumber(), point, self:GetCaster(), self:GetSpecialValueFor("radius"), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, 0, 0, false )
...
function modifier_genos_incineration_slow:DeclareFunctions()
  local funcs = {MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE}
  return funcs
end

function modifier_genos_incineration_slow:GetModifierMoveSpeedBonus_Percentage()
