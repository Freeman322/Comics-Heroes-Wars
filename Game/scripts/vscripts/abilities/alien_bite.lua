...
function modifier_alien_bite:OnAttackLanded(params)
    ...
    if (self:GetAbility():GetAutoCastState() or self.bite) and self:GetAbility():IsCooldownReady() and params.attacker == self:GetParent() and self:GetParent():IsRealHero() and (params.target:IsBuilding() and params.target:IsMagicImmune() and params.target:IsOther()) == false then
...
function modifier_alien_bite:GetModifierPreAttack_CriticalStrike()
    if (self:GetAbility():GetAutoCastState() or self.bite) and self:GetAbility():IsCooldownReady() then
...
function modifier_alien_bite:OnOrder(keys)
    if keys.unit == self:GetParent() then if keys.order_type == DOTA_UNIT_ORDER_CAST_TARGET and keys.ability:GetName() == self:GetAbility():GetName() then
        self.bite = true
    else
        self.bite = false
    end
