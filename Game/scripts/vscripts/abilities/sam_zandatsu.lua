...
function modifier_sam_zandatsu:OnDeath(params)
  if params.attacker == self:GetParent() and params.attacker:IsRealHero() and params.attacker:PassivesDisabled() == false and params.unit ~= params.attacker and params.unit:IsBuilding() == false then
    if params.unit:IsRealHero() then
      self:GetParent():Heal(self:GetParent():GetMaxHealth() * self:GetAbility():GetSpecialValueFor("hero_heal"), self:GetParent())
      self:GetParent():GiveMana(self:GetParent():GetMaxMana() * self:GetAbility():GetSpecialValueFor("hero_heal"))

    else
      self:GetParent():Heal(self:GetParent():GetMaxHealth() * self:GetAbility():GetSpecialValueFor("creep_heal"), self:GetParent())
      self:GetParent():GiveMana(self:GetParent():GetMaxMana() * self:GetAbility():GetSpecialValueFor("creep_heal"))
