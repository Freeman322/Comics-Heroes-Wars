...
{
    var pID = $.GetContextPanel().GetAttributeInt("player", -1)
    var info = Game.GetPlayerInfo( pID )

    $("#AvatarImage").accountid = info.player_steamid
    $("#UserName").text = info.player_name

    var rank = getPlayerRank(pID)
    $("#RankImage").style.backgroundImage = 'url("s2r://panorama/images/ranks/' + rank + '_png.vtex")';
    $("#RankImage").style.backgroundSize = 'cover;';
    var prestige = getPlayerPrestigeIcon(pID)
    var prestige_name = getPlayerPrestigeNumber(pID)

    if (prestige != 0 && prestige_name != ""){
        $("#PrestigeImage").visible = true; $("#PrestigeName").visible = true;

        $("#PrestigeImage").style.backgroundImage = 'url("s2r://panorama/images/prestige/' + prestige + '_png.vtex")';
        $("#PrestigeImage").style.backgroundSize = 'cover;';
        $("#PrestigeName").text = prestige_name
    }else { $("#PrestigeImage").visible = false;  $("#PrestigeName").visible = false; }

    var medal = getMedal(pID)
    if (medal != 0) { var icon = getMedalIcon(medal); $("#MedalImage").visible = true; $("#MedalImage").style.backgroundImage = 'url("s2r://panorama/images/econs/' + icon + '_png.vtex")'; $("#MedalImage").style.backgroundSize = 'cover;';} else { $("#MedalImage").visible = false; }

    if (getPlusSubscribe(pID) == false) { $("#CardOverlay").AddClass("close"); $("#PlusOverlay").visible = false; } else { $("#CardOverlay").RemoveClass("close")
        var winrate = getWinrate(pID)

        $("#Winrate").text = (winrate == "NaN%" ? "0%" : winrate)
        $("#Games").text = getGames(pID)
        $("#PlusOverlay").visible = true;
    }

    if (isPremium(pID) == true){ $("#PremiumStatus").visible = true; $("#UserName").AddClass("premium")
    }else { $("#PremiumStatus").visible = false; $("#UserName").RemoveClass("premium") }
}
